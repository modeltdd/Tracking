...
|---|---|---|---|
| A1 | ตัวตนที่ QR เก็บ | QR เก็บเฉพาะ `https://<web-app-url>/t/<qr_token>` โดย `qr_token` เป็นสตริงสุ่ม 8 ตัว **ไม่ใช่** `tracking_no` ตรงๆ — กันการเดา/ไล่เลขติดตามอื่นจาก QR ที่ถ่ายภาพหลุด | สร้างใหม่ได้ต่อ work_id ถ้า token รั่ว (ปุ่ม "ออก QR ใหม่" ใน Admin) |
