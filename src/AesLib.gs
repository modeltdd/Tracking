/**
 * Vendored AES (Rijndael) + AES-CTR implementation — (c) Chris Veness 2005-2019, MIT Licence
 * Source: https://github.com/chrisveness/crypto (aes.js + aes-ctr.js), retrieved 2026-08-17
 *
 * Apps Script has no native symmetric-cipher API, so this pure-JS reference implementation is
 * vendored in rather than hand-rolled. Logic is UNCHANGED from upstream except: `import`/`export`
 * statements removed (Apps Script concatenates .gs files into one global scope, no ES modules) and
 * both files merged into one to guarantee Aes loads before AesCtr regardless of file load order.
 * Environment glue (base64Encode/base64Decode — upstream assumes a browser or Node) is overridden
 * separately in Crypto.gs using Utilities.base64Encode/base64Decode, since GAS has neither
 * btoa/atob nor Buffer.
 *
 * Used only for `citizen_id_encrypted` (Works sheet) per docs/DESIGN.md A5 — reversible storage of
 * the full 13-digit citizen ID for Admin-only edit flows. Public/staff verification uses
 * `citizen_id_last4` (plain) and dedup uses `citizen_id_hash` (SHA-256, see Crypto.gs) — neither
 * needs this cipher.
 */
